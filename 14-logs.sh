...
Y="\e[33m"
N="\e[0m"
LOGS_FOLDER="/var/log/shellscript-logs" ## In general, log location /var/log/
SCRIPT_NAME=$(echo $0 | cut -d "." -f1)
LOG_FILE="$LOGS_FOLDER/$SCRIPT_NAME.log" 

mkdir -p $LOGS_FOLDER ## -p --> if folder is not created, then it creates, & if already created, then will ignore ; no error
echo "Script started executing at: $(date)" | tee -a $LOG_FILE # -a is append , 
# 'tee' appends to log file and shows on screen too i.e. std output. 
